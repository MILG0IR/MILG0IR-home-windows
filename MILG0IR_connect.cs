...
namespace MILG0IR_home_windows_x64 {
    public partial class MILG0IR_connect : Form {
        ...
        private void Connx(object sender, EventArgs e) {
            bool ready = true;
            string api = (ApiInput.Text == ApiInput.Name) ? null : ApiInput.Text;
            string uri = (UriInput.Text == UriInput.Name) ? null : UriInput.Text;
            if (uri == null && api != null) { MessageBox.Show("Please enter a valid server URI", "Connection error", MessageBoxButtons.OK, MessageBoxIcon.Warning); ready = false; } else
            if (uri != null && api == null) { MessageBox.Show("Please enter a valid server API key", "Connection error", MessageBoxButtons.OK, MessageBoxIcon.Warning); ready = false; } else
            if (uri == null && api == null) { MessageBox.Show("Please enter a valid server URI and API key", "Connection error", MessageBoxButtons.OK, MessageBoxIcon.Warning); ready = false; }
            if (ready && MILG0IR.IsUrl(uri)) {
                if (!uri.ToLower().StartsWith("http")) uri = "http://" + uri;
                if (!uri.ToLower().EndsWith("/")) uri += "/";
                uri += "api/index.php";
                if (MILG0IR.Connnect(uri, api)) {
                    Settings.Default.API_URI = uri;
